#ifndef FREAKAlgorithm_h
#define FREAKAlgorithm_h
/**
 * @file
 * $Revision: 7311 $ 
...
/**
 * @brief FREAK Feature matcher algorithm 
 *  
 * This class provides the OpenCV3 FREAK Feature2D algortithm. Only the 
...
class FREAKAlgorithm : public Feature2DAlgorithm {  // See FeatureAlgorithm.h
  //  OpenCV 3 API
  public:
    FREAKAlgorithm();
    FREAKAlgorithm(const PvlFlatMap &cvars, const QString &config = QString() );
    virtual ~FREAKAlgorithm();
  ...
  private:
    typedef cv::xfeatures2d::FREAK  FREAKType;
    typedef cv::Ptr<FREAKType>      FREAKPtr;
};
