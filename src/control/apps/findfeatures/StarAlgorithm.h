...
/**
 * @brief StarDetector Feature matcher algorithm 
 *  
 * This class provides the OpenCV3 StarDetector Feature2D algortithm. Only the 
...
class StarAlgorithm : public Feature2DAlgorithm {  // See FeatureAlgorithm.h
  //  OpenCV 3 API
  public:
    StarAlgorithm();
    StarAlgorithm(const PvlFlatMap &cvars, const QString &config = QString() );
  
    virtual ~StarAlgorithm();

    QString description() const;

    // Required for all algorithms
    static Feature2DAlgorithm *create(const PvlFlatMap &vars, const QString &config = QString());
  ...
  private:
    typedef cv::xfeatures2d::StarDetector  StarType;
    typedef cv::Ptr<StarType>              StarPtr;
};
